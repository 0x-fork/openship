...
import { ArrowDownUp, ArrowUpRight, Cpu, Globe2, HardDrive, MemoryStick, RefreshCw, Zap } from "lucide-react";
...
export function BillingResourceUsage({ state }: { state: BillingState }) {
  ...
  const computeRows = [
    { label: copy.cpu, hint: copy.cpuHint, used: compute?.cpuHours ?? null, unit: "vCPU-h", Icon: Cpu },
    { label: copy.memory, hint: copy.memoryHint, used: compute?.memoryGbHours ?? null, unit: "GB-h", Icon: MemoryStick },
    { label: copy.disk, hint: copy.diskHint, used: compute?.diskIoGb ?? null, unit: "GB", Icon: HardDrive },
    { label: copy.transfer, hint: copy.transferHint, used: compute?.networkGb ?? null, unit: "GB", Icon: ArrowDownUp },
  ];
