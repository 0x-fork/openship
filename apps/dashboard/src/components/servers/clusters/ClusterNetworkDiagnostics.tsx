...
export function ClusterNetworkDiagnostics({
  ...
  const c = t.servers.clusters,
    d = c.diagnostics;
  const links = useMemo(() => networkLinks(members, report), [members, report]);
    ...
    <section className="@container/network-diagnostics min-w-0 space-y-4" aria-label={d.title}>
      ...
      {compact && selected && (
        ...
        >
          ...
          <span className="min-w-0 flex-1">{d.connectionDetails}</span>
          <ChevronDown
      ...
      {selected && showDetails && (
        <div className="min-w-0 rounded-2xl bg-card p-5 sm:p-6">
            ...
            <div className="min-w-0 flex-1 basis-56">
              ...
              <CustomSelect
                ...
                options={links.map((link) => ({
                  value: link.id,
                  label: `${link.source.name} ↔ ${link.target.name}`,
                }))}
              />
            </div>
            {onSpeedTest && (
              <Button
                type="button"
                variant="secondary"
                disabled={speedDisabled || running}
                onClick={() =>
                  onSpeedTest({
          ...
          <div className="mt-5 grid gap-5 @2xl/network-diagnostics:grid-cols-2">
            {selected.directions.map(({ source, target, check, handshake, speed }) => (
              <div key={source.serverId} className="min-w-0 rounded-xl bg-muted/20 p-4">
                ...
                {(!compact || check || speed) && (
                  <dl className="mt-4 grid grid-cols-2 gap-4 border-t border-border/40 pt-4">
                    {[
                      [d.roundTrip, metric(check?.latencyKind === "rtt" ? check.latencyMs : null)],
