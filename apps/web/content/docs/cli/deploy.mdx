...
Triggers a deployment for the current project:

- **Inside a git repo** — builds the linked project from its git source.
- **With `--folder`** — packages the current folder and uploads it (the same folder-deploy pipeline the
  dashboard uses; see [Deploy a local folder](/docs/guides/deploy-local-folder)).

Outside Git, uploading requires `--folder` or `--name <name>`. A project ID alone never opts into
uploading the current directory. For a Git redeploy from another directory, pass `--branch <name>`.
Folder uploads refuse the system temporary directory and any ancestor of it; staging must stay
outside the source tree. Choose the app's source directory, or set `TMPDIR` outside it.

Passing a git-only flag (`--branch`, `--commit`, `--smart-route`, `--refresh`) selects the Git path.
These flags cannot be combined with `--folder`. `--service-ids` scopes either deployment path.
...
| `--folder`            | Upload the current directory, including when it is a Git checkout.                    |
...
<Callout title="Keep / reject" type="info">
  ...
  redeploy](/docs/guides/rollback-redeploy) for when this happens.
</Callout>
...
<Callout title="Follow vs tail">
  ...
  and monitoring, see [Logs & monitoring](/docs/guides/logs-monitoring).
</Callout>
