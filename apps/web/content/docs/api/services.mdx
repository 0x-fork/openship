...
The dashboard uses the same environment editor and **Show value / Hide value** controls when importing
Compose services and editing saved services. Revealing a value only changes its display; it does not
save or overwrite configuration.
...
During import, the same editor reads the source supplying the displayed values:

| Source | SDK | REST API |
| --- | --- | --- |
| Git or local scan | `deployments.revealPreparedEnv({ ...prepareInput, service: "db", keys: ["POSTGRES_PASSWORD"] })` | `POST /api/deployments/prepare/env-reveal` with the same body |
| Uploaded snapshot | `sources.reveal("session_123", { service: "db", keys: ["POSTGRES_PASSWORD"] })` | `POST /api/projects/folder/scan/session_123/env-reveal` with `{ "service": "db", "keys": ["POSTGRES_PASSWORD"] }` |

For Git or local scans, keep the source, branch, `composePath`, and interpolation `env` from `prepare`.
The lookup rereads source files, so values can change if the source changes. It requires deployment write
access and, for Git, whole-repository content read access. Local paths are self-hosted only. Upload
lookups read the authorized session's snapshot. All lookups share key validation and filtering.
