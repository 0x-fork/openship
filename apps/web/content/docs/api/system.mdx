---
title: System API
description: Self-hosted instance administration — onboarding, SSH servers, component install, tunnels, filesystem browse, team-mode migration, and scoped data transfer.
---
...
exporting or importing the instance or selected projects. In the dashboard this is spread across the onboarding wizard and
...
| `GET /api/system/data-transfer/preview` | `settings:admin` | List exportable projects and instance row counts (instance administrator). |
| `POST /api/system/data-transfer/preview` | `settings:admin` | Preview a project selection and its dependencies (instance administrator). |
| `POST /api/system/data-transfer/export` | `settings:admin` | Export the instance or selected projects (instance administrator). |
| `POST /api/system/data-transfer/import` | `settings:admin` | Import an export file, ≤500 MB, with scope and conflict options (instance administrator). |
| `POST /api/system/data-transfer/import/session/:sessionId/preview` | `settings:admin` | Preview an uploaded file's selection, conflicts, and server mappings. |
