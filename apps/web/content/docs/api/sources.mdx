...
```ts
const scan = await ship.sources.scan("session_123");
...
masked; use [Reveal environment values](/docs/api/services#reveal-environment-values) to display selected keys.
...
| Detect build configuration from a staged source session. | `sources.scan(id)` | `POST /api/projects/folder/scan/:sessionId`<br />`project:write` |
