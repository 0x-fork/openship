...
state = {'exists': False, 'addresses': [], 'routes': [], 'peers': [], 'publicKey': '', 'listenPort': None, 'enabled': False}
commands = []
...
with tempfile.TemporaryDirectory() as directory:
 ...
 common = {'managedId': identity, 'interfaceName': 'oswg' + 'a' * 10, 'stateDir': directory, 'operationId': '00000000-0000-4000-8000-000000000001', 'generation': 1, 'expectedConfigHash': None}
 config = {
 ...
 if before:
  ...
  state.update(exists=True, addresses=['10.243.0.1/32'], routes=['10.244.0.2/32'], peers=['peer-key'], publicKey='public:old-fixture-key', enabled=True)
 ...
 def snapshot(result):
  ...
  return {'result': result, 'state': copy.deepcopy(state), 'commands': copy.deepcopy(commands), 'receipt': json.loads((base / 'receipt.json').read_text()), 'transportOnly': bool(current and current.get('transportOnly'))}
...
async function run(scenario = "promote", existing = false) {
