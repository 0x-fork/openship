...
function fixtures(os: string, pageSize = 16384) {
  ...
  dirs.push(dir);
  const command = (name: string, body: string) => writeFileSync(join(dir, name), `#!/bin/sh\n${body}\n`, { mode: 0o755 });
  ...
  command("cat", `case "$1" in /proc/uptime) echo '3600.5 0.0' ;; /proc/loadavg) echo '1.2 2.3 3.4 1/2 3' ;; *) exec /bin/cat "$@" ;; esac`);
  return { command, env: { ...process.env, STATS_FIXTURE: dir, PATH: `${dir}:/usr/bin:/bin:/usr/sbin:/sbin` } };
}
function sample(env = process.env, shell = "/bin/sh") {
  return JSON.parse(execFileSync(shell, ["-c", SERVER_STATS_COMMAND], { env, encoding: "utf8", timeout: 12000 }));
}
...
describe("server monitor host command", () => {
  it.each([4096, 16384])("reads macOS interval CPU and memory with %i-byte pages", (pageSize) => {
    ...
    valid(stats);
    expect(stats).toMatchObject({ cpu: 40, memTotal: 17179869184, memAvail: 33000 * pageSize, diskTotal: 12345678901 * 1024, uptime: "3600", load1: "1.2", load5: "2.3", load15: "3.4" });
  });
  it("reads Linux counters including interrupts, steal and idle I/O wait", () => {
    const stats = sample(fixtures("Linux").env);
    valid(stats);
    expect(stats).toMatchObject({ cpu: 60, memTotal: 17179869184, memAvail: 4294967296, uptime: "3600.5" });
  });
  ...
  it("rejects unsupported hosts", () => {
    const r = spawnSync("/bin/sh", ["-c", SERVER_STATS_COMMAND], { env: fixtures("Unknown").env, encoding: "utf8" });
    expect(r.status).not.toBe(0);
    expect(r.stdout).toBe("");
  ...
  if (process.platform === "darwin") {
    it("also works through macOS's default zsh login shell", () => { valid(sample(process.env, "/bin/zsh")); });
  }
});
