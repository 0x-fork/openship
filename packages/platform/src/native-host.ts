...
export async function createNativePlatform(value: NativePlatformOptions): Promise<NativePlatform> {
  ...
  const projectResources = Object.fromEntries(["create", "ensure", "get", "list", "getHome", "update", "scanLocal", "importLocal", "listLocal", ...Object.keys(ProjectControlSchemas)].map(name => [name, (...args: unknown[]) => call(`projects.${name}`, ...args)])) as Omit<PlatformProjectOperations, "streamRuntimeLogs" | "streamClusterDatabaseEvents" | "openServerLogStream">;
